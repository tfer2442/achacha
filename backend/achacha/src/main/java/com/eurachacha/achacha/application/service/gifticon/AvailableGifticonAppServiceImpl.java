...
import com.eurachacha.achacha.infrastructure.util.PageableFactory;
...
public class AvailableGifticonAppServiceImpl implements AvailableGifticonAppService {
    ...
    public AvailableGifticonResponseDto getAvailableGifticons(GifticonScopeType scope, GifticonType type, GifticonSortType sort, Integer page, Integer size) {
        ...
        // 쿼리 실행
        Slice<AvailableGifticonDto> gifticonSlice = availableGifticonRepository.getAvailableGifticons(userId, effectiveScope, type, pageable);

        return AvailableGifticonResponseDto.builder()
                .gifticons(gifticonSlice.getContent())
                .hasNextPage(gifticonSlice.hasNext())
                .nextPage(gifticonSlice.hasNext() ? (page != null ? page : 0) : null)
                .build();
    }
}
